...
int solve() {
	point<long double> p0, p1;
	cin >> p0 >> p1;
	int q;
	cin >> q;
	for (int i = 0; i < q; ++i) {
		point<long double> p2;
		cin >> p2;
		int val = cross(p2 - p0, p1 - p0);
		if (val < 0) {
		...
		if (val == 0) {
			// check if pointing in opposite directions
			if ((p2 - p0).abs() == 0 || (p1 - p0).abs() == 0) {
				cout << "ON_SEGMENT\n";
			} else if ((p2 - p0).unit() == (p1 - p0).unit()) {
				if ((p2 - p0).abs() > (p1 - p0).abs()) {
					cout << "ONLINE_FRONT\n";
				} else {
					cout << "ON_SEGMENT\n";
				}
